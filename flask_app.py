...
@app.route('/sci_menu')
def sci_menu():
    return render_template('sci_menu.html')
...
def energy():
    ...
    return render_template('energy.html', values = values, mass = mass)


@app.route('/conservation_energy')
def conservation_energy():
    total_j = randint(30, 50)
    j=randint(15, 25)

    q=f'{total_j} Joules of energy enter a light bulb. {j} joules of energy are transformed into light, how much energy is dissipated as heat?'
    ans = round(total_j-j,2)

    return render_template('conservation_energy.html', q=q, ans=ans)
...
def mv():
    ...
    if (randint(1,2) == 1):
        question = f'If an aqueous solution is made with {m} g of a substance and {v} L of water. What is its concentration in %(m/V)?'
        ans = f'{round((m*100)/(v*1000),2)} %(m/v)'
    else:
        question = f'An aqueous solution of sodium chloride (salt) at {c} %(m/v) is used to preserve foods. To prepare a {v} L solution, how much salt must be dissolved in water?'
        ans = f'{round((c/100)*(v*1000),2)} g'

    return render_template('mv.html', q = question, ans = ans)
...
def permafrost():
    ...
    return render_template('permafrost.html', a=a, b=b, ans=ans)
...
def stoic():
    ...
    return render_template('stoic.html', equation = q, reac = reacs, reacVals = reacVals, prod = prods, prodVal = prodVals, mass = mass, mm0=mm0, mm1=mm1, ans = ans)
...
def ES_sec4():
    ...
    return render_template('ES_sec4.html')


@app.route('/MW_sec4')
def MW_sec4():

    return render_template('MW_sec4.html')


@app.route('/TW_sec4')
def TW_sec4():

    return render_template('TW_sec4.html')


@app.route('/speed_ratio')
def speed_ratio():
    r = randint(0, 1)
    r2 = randint(0, 1)
    a = randint(1, 5)
    ...
    if(r==0):
        ...
        question = 'its number of teeth'


    return render_template('speed_ratio.html', x=x, y=y, q=question, ans=ans)
...
def ohms_law():
    ...
    sol = f'Appliance A: {a} Ohms'
    sol2 = f'Appliance B: {a2} Ohms'

    return render_template('ohms_law.html', p1 = p1, p2 = p2, a = a, a2=a2, sol = sol, sol2 = sol2)
